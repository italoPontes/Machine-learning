...
def gradient_descent_runner(x, y, starting_w0, starting_w1, learning_rate, num_iterations, output_filename):
	...
	while (norma > 0.001 and not condiction) or ( iteration_number < num_iterations and condiction):
		...
		rss_by_step = compute_RSS(w0, w1, x, y)
...
def run(input_filename, output_filename, learning_rate, num_iterations):
	...
	if learning_rate == 0: 
		...
		print "Starting gradient descent at w0 = {0}, w1 = {1}, error = {2}".format(initial_w0, initial_w1, compute_RSS(initial_w0, initial_w1, x, y))
	...
	print "After {0} iterations w0 = {1}, w1 = {2}, error = {3}".format(num_iterations, w0, w1, compute_RSS(w0, w1, x, y))
